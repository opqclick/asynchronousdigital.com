            ...
            <div class="card-body">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('billing_model') is-invalid @enderror" id="billing_model" name="billing_model" required>
                                <option value="fixed_price" {{ old('billing_model', $project->billing_model) === 'fixed_price' ? 'selected' : '' }}>Fixed Price</option>
                                <option value="monthly" {{ old('billing_model', $project->billing_model) === 'monthly' ? 'selected' : '' }}>Monthly</option>
                                <option value="task_based" {{ old('billing_model', $project->billing_model) === 'task_based' ? 'selected' : '' }}>Task Based</option>
                            </select>
                            @error('billing_model')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" class="form-control @error('project_value') is-invalid @enderror" 
                                   id="project_value" name="project_value" value="{{ old('project_value', $project->project_value) }}">
                            @error('project_value')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="form-group">
                    ...
                    <input type="url" class="form-control @error('repository_url') is-invalid @enderror" 
                           id="repository_url" name="repository_url" value="{{ old('repository_url', $project->repository_url) }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 @error('teams') is-invalid @enderror" id="teams" name="teams[]" multiple>
                        @foreach($teams as $team)
                            <option value="{{ $team->id }}" {{ in_array($team->id, old('teams', $project->teams->pluck('id')->toArray())) ? 'selected' : '' }}>
                                {{ $team->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    @if($project->attachments && count($project->attachments) > 0)
                        <div class="mb-2">
                            <small class="d-block text-muted">Existing: {{ count($project->attachments) }} file(s)</small>
                        </div>
                    @endif
