        ...
        <form action="{{ route('admin.users.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card-body">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('active_role_id') is-invalid @enderror"
                                ...
                                <option value="">Auto (first selected role)</option>
                                @foreach($roles as $role)
                                    <option value="{{ $role->id }}" {{ old('active_role_id') == $role->id ? 'selected' : '' }}>
                                        {{ ucfirst(str_replace('_', ' ', $role->name)) }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <small class="form-text text-muted">Max 5MB per file (PDF, DOC, DOCX, Images). Multiple files allowed</small>
                            @error('documents.*')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('bank_swift_code') is-invalid @enderror" 
                                   id="bank_swift_code" name="bank_swift_code" value="{{ old('bank_swift_code') }}">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('payment_model') is-invalid @enderror" 
                                ...
                                <option value="hourly" {{ old('payment_model') == 'hourly' ? 'selected' : '' }}>Hourly</option>
                                <option value="fixed" {{ old('payment_model') == 'fixed' ? 'selected' : '' }}>Fixed</option>
                                <option value="monthly" {{ old('payment_model') == 'monthly' ? 'selected' : '' }}>Monthly</option>
                            </select>
                            @error('payment_model')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                ...
                <div class="form-group">
                    ...
                    <small class="form-text text-muted">Optional: assign one or more teams for this user.</small>
                    @error('teams')
                        <span class="invalid-feedback d-block">{{ $message }}</span>
                    @enderror
                    @error('teams.*')
                ...
                <div class="form-group">
                    <div class="custom-control custom-checkbox">
                        ...
                        <small class="form-text text-muted">Send login credentials to the user's email address</small>
                    </div>
                </div>
            </div>
